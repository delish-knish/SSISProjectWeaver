CREATE TABLE [cfg].[ETLPackageGroup_SQLCommand]
  (
    ...
    ,[LastUpdatedUser]              VARCHAR (50) CONSTRAINT [DF_ETLPackageGroup_SQLCommand_LastUpdatedUser] DEFAULT (SUSER_SNAME()) NOT NULL
     ...
     CONSTRAINT [FK_ETLPackageGroup_SQLCommand_SQLCommand_ETLPackageGroup] FOREIGN KEY ([ETLPackageGroupId]) REFERENCES [cfg].[ETLPackageGroup]([ETLPackageGroupId]),
     CONSTRAINT [FK_ETLPackageGroup_SQLCommand_SQLCommand_SQLCommand] FOREIGN KEY (SQLCommandId) REFERENCES [cfg].[SQLCommand]([SQLCommandId]),
  )
