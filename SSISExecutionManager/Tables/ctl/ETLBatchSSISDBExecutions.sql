CREATE TABLE [ctl].[ETLBatchSSISDBExecutions]
  (
     ...
     CONSTRAINT [PK_ETLBatchSSISDBExecutions] PRIMARY KEY ([ETLBatchId], [SSISDBExecutionId]),
     CONSTRAINT [FK_ETLBatchSSISDBExecutions_ETLPackage] FOREIGN KEY ([ETLPackageId]) REFERENCES ctl.ETLPackage([ETLPackageId]),
     CONSTRAINT [FK_ETLBatchSSISDBExecutions_ETLBatchId] FOREIGN KEY (ETLBatchId) REFERENCES ctl.ETLBatch(ETLBatchId),
  )
